...
<<describeit,rgl=TRUE,eval=TRUE,echo=TRUE,warning=FALSE,message=FALSE, dev='png', fig.width=4, fig.height=4, out.width='.5\\linewidth'>>=
opts_chunk$set(fig.path='figure/Eigenanatomy1-', fig.align='center', fig.show='hold')
####################################################
...
for ( i in mysig )
  ...
  cap<-paste('EigenNetwork',i,sep='')
  mytbl<-xtable(signifnetworkdescriptor$network, caption =cap, label = cap)
  print(mytbl)
  }
@ 
