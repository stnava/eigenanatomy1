...
basis vectors.  Note that PCA, owing to its gaussianity assumption identifies the true sources only up to a rotation, so we need a
...
prior neuroanatomical guidance.  EANAT component images, on the other hand, {enable prior knowledge to enhance solution stability} and are tied to a set of neuroanatomical coordinates that are {\em connected}, {\em smooth} and may also be defined by {\em non-negative} weights. 
...
#  statdir=basedire,smooth=0.0, z=usp)
  ...
  antsImageWrite(decom1, 'decom1/umat.nii.gz')
  ...
  decom1 <- as.matrix(antsImageRead('decom1/umat.nii.gz', 2))
  mydecom <- list(eigenanatomyimages=eigenanatomyimages, umat=decom1)
}
...
\begin{figure}[h]
  \centering
    \includegraphics[width=0.95\textwidth]{figure/within_network_correlation.pdf}
...
for ( ii in c(1:nrow(mp)) ) { 
  ...
  if ( ii < (nrow(mp)-1))
    if(decompose) {
      ...
      decom1 <- mydecom$umatrix
      for(i in 1:length(mydecom$eigenanatomyimages)){
        antsImageWrite(mydecom$eigenamatomyimages[[i]], 
          paste(basedire, '/eigenanatomyimage', sprintf('%02d', i), '.nii.gz', sep=''))
      }
      antsImageWrite(decom1, paste(basedire, 'umat.nii.gz', sep=''))
    } else {
      eigenanatomyimages <- list()
      names.eigimg <- list.files(basedir, pattern=glob2rx('eigenanatomyimage*.nii.gz'), full.names=T)
      for(i in 1:length(names.eigimg)){
        eigenanatomyimages[[i]] <- antsImageRead(names.eigimg[i], 3)
      }
      decom1 <- as.matrix(antsImageRead(paste(basedir, 'umat.nii.gz', sep=''), 2))
      mydecom <- list(eigenanatomyimages=eigenanatomyimages, umat=decom1)
    }
