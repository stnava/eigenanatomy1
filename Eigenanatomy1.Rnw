...
# global settings to control what to recompute
booleval <- TRUE
decompose <- F # takes ~ 45 minutes--must be done at least once in a fresh clone
render <- F # similarly, must be done at least once 

options(xtable.comment = FALSE)
options(replace.assign=TRUE,width=90)
options( digits = 4 )
opts_chunk$set(eval=booleval, echo = FALSE, warning=FALSE, message=FALSE, digits=4)
...
\begin{abstract}
We present ``Eigenanatomy,'' a unified formulation for sparse component analysis of
...
signal.  Data-driven dimensionality reduction and feature selection techniques
provide a potentially optimal strategy for analyzing ``big data" in
biological domains CITATIONS.  Dimensionality reduction methods find
...
Although there are many variants of dimensionality reduction, the three most representative and widely used techniques are Principal Component Analysis (PCA), Non-negative Matrix Factorization (NMF), and Independent Component Analysis (ICA).  (We limit ourselves here to linear dimensionality reduction techniques, as non-linear techniques \cite{roweis_nonlinear_2000,saxena_non-linear_2004,donoho_hessian_2003} are generally more complex and suspect to noise.)   PCA \cite{jolliffe2005principal} is one of the most widely used dimensionality reduction algorithms.  

NMF~\cite{paatero1994positive}  constrains the learned components to be
positive, with the result that each  component of the dictionary is a positive
...
data into more interpretable pieces~\cite{Lee1999,sparseNMF_hoyer,lee_learning_1999,berry2007algorithms}.  Sparse versions of NMF have also been described~\cite{sparseNMF_hoyer}.

ICA~\cite{hyvarinen2000independent} has a different history and notation from PCA and NMF, and was originally motivated by the ``blind source separation'' problem: Given a data matrix that
contains information from a variety of sources, how can we uncover the
original sources?  As opposed to PCA, which optimizes variance explained and orthogonality among the basis vectors, ICA optimizes ``independence'' or ``non-Gaussianity'' of the 
basis vectors.  PCA, owing to its Gaussianity assumption identifies the true sources only up to a rotation, whereas the assumption of non-Gaussian sources in ICA enables recovery of sources without the rotational ambiguity. 

Traditional PCA has the disadvantage that
...
the low-dimensional space difficult.  This limits their popularity especially among clinicians who
are often as interested in clinical interpretability as predictive
accuracy. Sparse methods have sought to resolve this issue~\cite{sparseNMF_hoyer,Witten2010,Friedman2010,Cherkassky2009,Friedman2008}. 
One common method is Sparse PCA (SPCA)~\cite{jolliffe_simplified_2000,shen2008sparse,guan2009sparse,ulfarsson_sparse_2007}.  Sparse
...
coding~\cite{Lee2006,Mairal2010} (or dictionary learning)~\cite{varoquaux2011multi,abraham2013extracting}.  Sparse techniques may mimic how the human cortex processes information~\cite{Olshausen2004}.  

Using standard sparse techniques for spatially structured neuroimaging data is suboptimal for several reasons.  First, anatomically uninformed sparse techniques allow for anatomically implausible solution vectors consisting of scattered, unconnected voxels that are correlated with an outcome.  Second, it has recently been shown that sparse methods are necessarily unstable \cite{Xu2011}.  Informally, the argument is as follows:  If a method is \textit{sparse} in the sense that it will choose only one of two equally strong predictors, the method must be unstable because either one of the predictors is equally likely to be chosen, and there are thus multiple equally optimal solutions.  Incorporating anatomically reasonable constraints in the solution, such as smoothness and minimum cluster size, can ameliorate the instability of the sparse solution by providing additional anatomical information that will help the algorithm choose which one of two equally strong predictor voxels to include in the solution vector.  

Several groups have proposed methods to generate anatomically informed sparse decompositions.  These have included hierarchical Ward clustering \cite{michel_supervised_2012}, sparse regression techniques \cite{grosenick_interpretable_2008,grosenick_interpretable_2013,grosenick_whole-brain_2009,kandel_predicting_????,varoquaux_small-sample_2012,sabuncu_relevance_2012}, and a combined sparse decomposition and regression technique \cite{batmanghelich_general_2009,batmanghelich_regularized_2011,batmanghelich_generative-discriminative_2012}.  In addition, recent work points to the fact that exploiting problem-specific information can improve parts-based representations \cite{Guan2011,Cai2010,Hosoda2009}.  To date, though, a careful comparison of PCA, NMF, and ICA has been lacking. Each algorithm has a different history, employs different theoretical arguments and is favored in a different community. Comparing these methods is challenging, as it is difficult to evaluate the relative importance of the fundamental decomposition objective function, the regularization terms, and the optimization procedure.  In particular, ICA's terminology and assumptions differ significantly from PCA and NMF. In this paper, we propose to bridge this gap by providing a unified framework that encompasses all the commonly used decomposition techniques, thus both clarifying the theoretical differences between the techniques and shedding light on the practical advantages and disadvantages of each one.

Recently, questions have been raised about the methods and interpretation of the various decomposition methods, and specifically whether the ``independence'' or the sparsity of the sources drives the ICA decomposition \cite{daubechies_independent_2009,calhoun_independent_2013}.  In addition, there has been a recent interest in the interpretability of the components from the various decompositions.  In particular, studies have pointed out that naively looking at the weights of the components is inappropriate \cite{haufe_interpretation_????}, as the coefficients, or loadings, are at least at significant as the weights themselves.  There has also been a re-evaluation of the assumptions underlying sparse coding and regression techniques, as regressions tend to perform nearly as well on voxels not included in the sparse support of the regression vectors as on the selected voxels \cite{konukoglu_feature_2013}.  This observation has emphasized the need for principled statistical analysis of the output of dimensionality reduction techniques, as a naive interpretation of the raw output can be uninformative and even misleading.  Therefore, a second aim of this work is to demonstrate how the output of the decomposition techniques can be used in a traditional ($n > p$) linear model framework.  Using dimensionality reduction to convert the original $n \ll p$ problem to a traditional $n > p$ problem, or ``clustering, then statistics,'' increases the power of observational studies (CITE COREY'S POWER PAPER) and greatly simplifies statistical inference by avoiding the multiple comparisons problem \cite{nichols_controlling_2003}.  In addition, working with an $n < p$ problem makes it possible to use linear models to generate predictions for unseen data, thus minimizing the possibility of overfitting to a dataset.   
...
\item Customization of Eigenanatomy for neuroimaging by using domain specific
sparsity and smoothness constraints which aid interpretability and reproducibility of
results and give superior predictive performance.
\item A thorough evaluation involving comparison with standard
approaches. 
\item Public availability of our toolkit.
\end{enumerate}
...
bm_smooth <- antsImageClone(bm)
SmoothImage(3, bm, 2, bm_smooth)
...
#  mydecom<-sparseDecom( as.matrix(mydata_sub[,3:ncol(mydata_sub)]), mask,  sparseness=vsp, nvecs=nvecs, its=its, cthresh=cl,       
#  statdir=basedire,smooth=0.0, z=usp)
  decom1<-mydecom$umatrix
 
  for (i in 1:length(mydecom$eigenanatomyimages)){
    antsImageWrite(mydecom$eigenanatomyimages[[i]], paste('decom1/eigenanatomyimage', sprintf('%02d', i), '.nii.gz', sep=''))
  }
  write.csv(decom1, 'decom1/umat.csv', row.names=F)
  ...
  eigenanatomyimages <- list()
  eigParcellation <- antsImageRead('tempoutput/eigseg.nii.gz', 3)
  names.eigimg <- list.files('decom1', pattern=glob2rx('eigenanatomyimage*.nii.gz'), full.names=T)
  for( i in 1:length(names.eigimg)){
    eigenanatomyimages[[i]] <- antsImageRead(names.eigimg[i], 3) 
  }
  decom1 <- read.csv('decom1/umat.csv') 
  mydecom <- list(eigenanatomyimages=eigenanatomyimages, umatrix=decom1)
}
...
if(!file.exists('decom2')){
  decompose <- T
  dir.create('decom2')
}
if(decompose) {
  mydecom2<-sparseDecom2( inmatrix=list(myproj, cogmat ), sparseness=c(0.2,-0.5), nvecs=5, its=20, perms=2000 )
  for ( name in names(mydecom2)){
    write.csv(mydecom2[name], paste('decom2/', name, '.csv', sep=''), row.names=F)
  }
} else {
  require(tools)
  mydecom2 <- list()
  names.csv <- list.files('decom2', pattern=glob2rx('*.csv'), full.names=T)  
  for (i in 1:length(names.csv)) {
    mydecom2[[ i ]] <- read.csv(names.csv[i]) 
  }
  names(mydecom2)<- basename(file_path_sans_ext(names.csv)) 
}
# report correspondences for 1st 2 evecs
ind<-4
wh1<-which( mydecom2$eig1[,ind] > 0 )
wh2<-which( mydecom2$eig2[,ind] > 0 )
print( wh1 )
...
<<figsetup,echo=TRUE,results='hide',warning=FALSE,message=FALSE, echo=FALSE>>=
#
...
mysig<-order(qv)
if(render){
  brain<-renderSurfaceFunction( surfimg =list( bm_smooth ) ) 
  id<-par3d("userMatrix")
...
<<describeit, echo=FALSE, results='hide',dev='png', fig.width=4, fig.height=4, out.width='.5\\linewidth'>>=
...
if(length(list.files('figure', pattern=glob2rx('network1*.png'))) == 0)  render <- T
if(render){
  dir.create('figure') 
  brain<-renderSurfaceFunction( surfimg =list( bm_smooth ) , 
    alphasurf=0.1 ,smoothsval = 1.5 , smoothfval = 1.0, 
    funcimg=list(cnt$clustimg) , alphafunc=0.2 )
# brain <-renderSurfaceFunction( list( temgm ), surfval=0.5 , smoothsval=2., smoothfval = 0,mycol=mycols, funcimg=list(f1,f2,f3,glass),   
# alphafunc = myopac )
...
<<netcorr,results='hide',echo=FALSE>>=
...
library(pheatmap)
pdf('figure/within_network_correlation.pdf')
...
<<spider,results='hide',echo=FALSE>>=
row.names(clustmat)<-subjin$EMMI
...
<< describeit2,rgl=FALSE,results='hide',dev='png', fig.width=4, fig.height=4, out.width='.5\\linewidth' >>=
opts_chunk$set(fig.path='figure/Eigenanatomy1-', fig.align='center', fig.show='asis',fig.keep='all')
#$
ct<-2
for ( i in mysig[2:4] ){
  ...
  if(render) {
    brain<-renderSurfaceFunction( surfimg =list( bm ) , alphasurf=0.1 ,
      smoothsval = 1.5 , smoothfval = 1.0, funcimg=list(cnt$clustimg) , alphafunc=0.2 )
    plotBasicNetwork( centroids =  cnt$centroids , brain )
    par3d(userMatrix = id ) 
...
resultlist<-list()
resultlist2<-list()
if(!file.exists('eanat_processing1')) decompose <- T
  ...
  if ( ii < (nrow(mp)-1)){
    if(decompose) {
      mydecom<-sparseDecom( (mat), mask,  sparseness=mp$vSparseness[ii],
        nvecs=nvecs, its=its, cthresh=5550, statdir=basedire,smooth=0, z=mp$uSparseness[ii])
      decom1 <- mydecom$umatrix
      for(i in 1:length(mydecom$eigenanatomyimages)){
        antsImageWrite(mydecom$eigenanatomyimages[[i]], 
          paste(basedire, '/eigenanatomyimage', sprintf('%02d', i), '.nii.gz', sep=''))
      }
      write.csv(decom1, paste(basedire, '/umat.csv', sep=''), row.names=F)
    } else {
      eigenanatomyimages <- list()
      names.eigimg <- list.files(basedire, pattern=glob2rx('eigenanatomyimage*.nii.gz'), full.names=T)
      for(i in 1:length(names.eigimg)){
        eigenanatomyimages[[i]] <- antsImageRead(names.eigimg[i], 3)
      }
      decom1 <- read.csv(paste(basedire, '/umat.csv', sep=''))
      mydecom <- list(eigenanatomyimages=eigenanatomyimages, umatrix=decom1)
    }
...
\bibliography{icapca,kandel_bib}
\end{document}
